 ...
 *
 */
public class MttoBEMRenovToken extends ContractAbstractMB {
	
	private Integer mttoType;
	private String bemnumber;
	private String quantityAccounts;
	
	private String newTokenNumber_1;			//AttrConstants.MTTO_NEW_TOKEN_NUMBER_1    
	...
	private String templateSolicitud_renovToken;
	 ...
	 * 
	 */
	public MttoBEMRenovToken() {
		super();
		this.mttoType = MttoType.REPOSICION_TOKENS.getMttoTypeId();
	...
	public void setResetForm() {
		...
		clearFields();
		setProduct(productBean.findById(new Integer(ProductType.MTTO_REPOSICION_TOKEN.value()))); 
	...
	public void setEditForm() {
		...
		String CD_FILLS[] = { AttrConstants.BEM_NUMBER,AttrConstants.CR_NUMBER,AttrConstants.CLIENT_SIC,
				...
				AttrConstants.QTY_ACCOUNTS,
				AttrConstants.MTTO_NEW_TOKEN_NUMBER_1,
				...
				AttrConstants.MTTO_TOKEN_NUMBER_1,
				AttrConstants.MTTO_TOKEN_NUMBER_2,AttrConstants.MTTO_TOKEN_NUMBER_3,
				AttrConstants.MTTO_TOKEN_NUMBER_4,AttrConstants.MTTO_TOKEN_NUMBER_5,
				AttrConstants.MTTO_FOLIO_PSD_1,AttrConstants.MTTO_FOLIO_PSD_2,AttrConstants.MTTO_FOLIO_PSD_3,
				AttrConstants.MTTO_FOLIO_PSD_4,AttrConstants.MTTO_FOLIO_PSD_5,
	...
	public boolean savePartialContract() {
		...
		addToList(list,AttrConstants.MTTO_TEMPLATE_SOLICITUD_RENOV_TOKEN, getTemplateSolicitud_renovToken());
...
/*	FacesContext fCtx 				= FacesContext.getCurrentInstance();
	...
	public String processInfo() {
		...
		this.setTemplateSolicitud_renovToken(ApplicationConstants.OPCION_SELECTED); 
		this.setInfoAccounts();
		validatePrevToken();
		return saveCompleteMtto();
	}
	public void validatePrevToken(){
...
//		ContractMessageErrors errors = new ContractMessageErrors();
		int accs = Integer.parseInt(quantityAccounts);
			...
			if(tokenNumber_1.length()!=9 || newTokenNumber_1.length()!=9){
				ContractMessageErrors errors = new ContractMessageErrors();
				errors.setMessage("El token 1 ANTERIOR y/o NUEVO debe ser de 9 posiciones");
				this.getGeneralInfoErrorsList().add(errors);
			}
			if((tokenNumber_2.length()!=9 || newTokenNumber_2.length()!=9) && accs>=2){
				ContractMessageErrors errors = new ContractMessageErrors();
				errors.setMessage("El token 2 ANTERIOR y/o NUEVO debe ser de 9 posiciones");
				this.getGeneralInfoErrorsList().add(errors);
			}
			if((tokenNumber_3.length()!=9 ||newTokenNumber_3.length()!=9)&& accs >=3){
				ContractMessageErrors errors = new ContractMessageErrors();
				errors.setMessage("El token 3 ANTERIOR y/o NUEVO debe ser de 9 posiciones");
				this.getGeneralInfoErrorsList().add(errors);
			}
			if((tokenNumber_4.length()!=9 ||newTokenNumber_4.length()!=9)&& accs>=4){
				ContractMessageErrors errors = new ContractMessageErrors();
				errors.setMessage("El token 4 ANTERIOR y/o NUEVO debe ser de 9 posiciones");
				this.getGeneralInfoErrorsList().add(errors);
			}
			if((tokenNumber_5.length()!=9 ||newTokenNumber_5.length()!=9)&& accs>=5){
				ContractMessageErrors errors = new ContractMessageErrors();
				errors.setMessage("El token 5 ANTERIOR y/o NUEVO debe ser de 9 posiciones");
				this.getGeneralInfoErrorsList().add(errors);
			}
	...
	public String getTemplateSolicitud_renovToken() {
		return templateSolicitud_renovToken;
	}

	public void setTemplateSolicitud_renovToken(String templateSolicitud_renovToken) {
		this.templateSolicitud_renovToken = templateSolicitud_renovToken;
	}
	public void metodoParaHacerMasPeso2(){
